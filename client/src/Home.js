    ...
    for (let a = 0; a < data.length; ++a) {
      if (data[a].length === 0) {
        ...
        if (
          ...
          ["*", "-", "+"].includes(data[a + 1].trim()[0]) === true ||
          (a !== data.length - 1 &&
            data[a + 1].trim().split(". ").length >= 2 &&
            isNaN(parseInt(data[a + 1].trim().split(". ")[0])) === false)
        ) {
          newData.push("");
          continue;
        }
        ...
        for (let b = 0; b < split_for_url.length; ++b) {
          if (this.validURL(split_for_url[b])) {
            if (
              !split_for_url[b].startsWith("https://") &&
              !split_for_url[b].startsWith("http://")
            ) {
              split_for_url[b] = "https://" + split_for_url[b];
            }
            let new_url = split_for_url[b]
              .replace("https://", "")
              .replace("http://", "")
              .replace("www.", "");
        ...
        if (
          check_number_list.length >= 2 &&
          isNaN(parseInt(check_number_list[0])) === false
        ) {
          newData.push(data[a]);
          /*
					check cases like:
          ...
          if (a !== data.length - 1) {
            ...
            if (
              check_number_list_next.length < 2 ||
              (check_number_list_next.length >= 2 &&
                isNaN(parseInt(check_number_list_next[0])))
            ) {
              newData.push("");
            }
          }
        ...
        if (
          ...
          (data[a + 1].trim().split(". ").length < 2 ||
            (data[a + 1].trim().split(". ").length >= 2 &&
              isNaN(parseInt(data[a + 1].trim().split(". ")[0]))))
        ) {
          newData.push(data[a] + "\\");
        } else {
          newData.push(data[a]);
  ...
  render() {
    return (
      <div>
          ...
          {this.state.isType === "file" && (
            <div style={{ width: "250px" }}>
              {this.state.infos !== null &&
                this.state.owner === this.state.infos.owner &&
                this.state.path.includes("/file/") === false && (
                  <div>
                    <MenuItem
                      onClick={() => {
                        this.remove();
        ...
        {/* change account */}
        <Modal
          show={this.state.showModalAccount}
        ...
        >
          ...
          >
            <div>
              {this.state.edit === false ? (
                <Note
                  text={this.parseMarkdown(this.state.textNote)}
                  style={{
                    width: "100%",
                    minHeight: "100%",
                    borderRadius: "6px",
          ...
          <Modal.Footer>
            {this.state.edit === false ? (
              ...
              <Row>
                <div>
                  {this.state.savingNote === false ? "Saved!" : "Saving..."}
                </div>
                <Button
                  variant="contained"
                  style={{
        ...
        {/* ask for password */}
        <Modal
          show={this.state.showModalPassword}
          onHide={this.closeModal}
          size="md"
          aria-labelledby="contained-modal-title-vcenter"
          centered
                ...
                >
                  <Upload
                    {...{
                      disabled:
                        this.getParent() === "/" ||
                        this.state.disableBottons === true
                          ? true
                          : false,
                      name: "file",
              ...
              this.state.notes
                .filter((item) => {
                  if (this.state.search.length > 0) {
                    let re = new RegExp(this.state.search.toLowerCase(), "i");
                    return (
                      re.test(item.title.toLowerCase()) ||
                      re.test(item.text.toLowerCase())
                    );
                  } else {
                    return true;
                  }
                ...
                .map((item, idx) => {
                  return (
                    <div
                      ...
                      key={item._id}
                      onMouseEnter={() =>
                        this.setState({ indexMouseOverNote: idx })
                      }
                      onMouseLeave={() =>
                        this.setState({ indexMouseOverNote: -1 })
                      }
                    >
                      <div
                        props={item}
                      ...
                      >
                        {item.type.startsWith("image") ? (
                          <img
                            ...
                            height="210"
                            src={
                              this.state.previewImg.length - 1 >= idx &&
                              this.state.previewImg[idx] !== null
                                ? this.state.previewImg[idx]
                                : ""
                            }
                          />
                        ) : (
                          <i
                            className={this.getMineType(item.type)}
